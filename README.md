...
## Try it
First you need to deploy the simple-server that will print all headers from incoming requests:
```
oc apply -f resources/simple-server/00-deploy.yaml -n sontaflow-infra
```
You can rebuild the image of the simple-server using the [Dockerfile](resources/simple-server/Dockerfile) and the [source script](resources/simple-server/simple-server.py).
...
Then check the logs for the `simple-server` pod:
```
================ Headers for first ================
...
Host: simple-server-service.sonataflow-infra
